...
parser = argparse.ArgumentParser(description="Evaluate a phase-2 or phase-3 COLA checkpoint.")
parser.add_argument(
    "--task",
    type=str,
    default="cola_phase_3_student_left_fixed_bar",
    help="Registered collaboration task.",
)
...
def evaluate():
    ...
    if hasattr(env_cfg, "bar_controller") and hasattr(
        env_cfg.bar_controller, "gain_randomization_enabled"
    ):
        env_cfg.bar_controller.gain_randomization_enabled = False
    ...
    runner = OnPolicyRunnerWholePipeResi(env, agent_cfg.to_dict(), log_dir=log_dir, device=agent_cfg.device)
